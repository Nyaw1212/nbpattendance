...
function getMovementBootstrap() {
  ...
  return { personnel: data.personnel, offices: data.offices, pending: listPendingMovements_() };
}
...
function createPersonnelMovement(payload) {
  ...
  try {
    ...
    stmt = conn.prepareStatement([
      ...
      '(movement_id, employee_key, full_name_snapshot, from_camp, from_office, to_camp, to_office, admin_order_no, admin_order_date, effective_date, remarks, status, created_by)',
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULLIF(?, '')::date, ?::date, NULLIF(?, ''), 'PENDING_RECEIPT', ?)"
    ].join(' '));
    const vals = [movementId, employeeKey, person.fullName, person.camp, person.office, toCamp, toOffice, adminOrderNo, adminOrderDate, effectiveDate, remarks, getMovementUserEmail_()];
...
function receivePersonnelMovement(movementId) {
  ...
  if (stillAtOrigin) updatePersonnelAssignmentInSheet_(movement.employeeKey, movement.toCamp, movement.toOffice);
...
function listPendingMovements_() {
  ...
  try {
    ...
    stmt = conn.prepareStatement([
      ...
      "admin_order_no, COALESCE(admin_order_date::text, ''), effective_date::text, COALESCE(remarks, ''), status,",
...
function movementRowToObject_(rs) {
  return { movementId:String(rs.getString(1)), employeeKey:String(rs.getString(2)), fullName:String(rs.getString(3)), fromCamp:String(rs.getString(4)), fromOffice:String(rs.getString(5)), toCamp:String(rs.getString(6)), toOffice:String(rs.getString(7)), adminOrderNo:String(rs.getString(8)), adminOrderDate:String(rs.getString(9)||''), effectiveDate:String(rs.getString(10)), remarks:String(rs.getString(11)||''), status:String(rs.getString(12)), createdBy:String(rs.getString(13)||''), createdAt:String(rs.getString(14)||'') };
}
...
function getPersonnelAssignmentFromSheet_(employeeKey) {
  ...
  for (let i = 1; i < values.length; i++) if (String(values[i][badgeCol]).trim() === employeeKey) return { row:i+1, camp:String(values[i][campCol]).trim(), office:String(values[i][officeCol]).trim(), campCol:campCol+1, officeCol:officeCol+1 };
  return null;
}

function updatePersonnelAssignmentInSheet_(employeeKey, camp, office) {
  const sheet = getSpreadsheet_().getSheetByName('LIST');
