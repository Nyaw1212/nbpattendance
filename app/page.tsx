...
function caseLabel(cell: Cell) { if (cell.status === 'LEAVE') return cell.leaveType ? leaveCode(cell.leaveType) : 'LEAVE'; return cell.status; }
...
export default function Home() {
  ...
  const abnormalCases = useMemo(() => officeRoster.map(person => ({ person, cell: cells[key(person.badgeNumber, activeDay)] || { status: 'UNRECORDED' as Status } })).filter(item => item.cell.status !== 'PRESENT' && item.cell.status !== 'OFF').sort((a, b) => a.cell.status.localeCompare(b.cell.status) || a.person.fullName.localeCompare(b.person.fullName)), [officeRoster, cells, activeDay]);
  ...
  async function loadWeek() {
    ...
    try {
      ...
      const merged = { ...seeded }; for (const rec of data.records || []) merged[key(String(rec.employee_key), String(rec.attendance_date))] = { status: rec.status, leaveType: rec.leave_type || undefined };
  ...
  function cycleCell(person: Personnel, day: Date) { setActiveDay(iso(day)); const k = key(person.badgeNumber, iso(day)); const current = cells[k]?.status || 'UNRECORDED'; const next = STATUS_ORDER[(STATUS_ORDER.indexOf(current) + 1) % STATUS_ORDER.length]; setCells(prev => ({ ...prev, [k]: { status: next, leaveType: next === 'LEAVE' ? prev[k]?.leaveType : undefined } })); }
  ...
  return <main className="page-shell"><section className="app-card">
    ...
    <div className="toolbar"><button onClick={() => moveWeek(-7)}>‹ Previous</button><strong>{week[0].toLocaleDateString()} – {week[6].toLocaleDateString()}</strong><button onClick={() => moveWeek(7)}>Next ›</button><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search personnel..." /><span className="hint">P → O → L → OB → A → — · ⋯ = optional details</span><button onClick={clearWeek}>Clear Week</button></div>

    <div className="grid-wrap">{!office ? <div className="empty">Select a camp and office.</div> : <table className="attendance-grid"><thead><tr><th>Personnel</th>{week.map(d => <th key={iso(d)} className={iso(d) === activeDay ? 'active-day-column' : ''}><button className="day-head-button" onClick={() => setActiveDay(iso(d))}><span>{d.toLocaleDateString('en-US', { weekday: 'short' }).toUpperCase()}</span><small>{d.getMonth() + 1}/{d.getDate()}</small></button></th>)}</tr></thead><tbody>{roster.map(p => <tr key={p.badgeNumber}><td><b>{p.fullName}</b><small>{p.rank} · Badge {p.badgeNumber || '—'}</small></td>{week.map(day => { const c = cells[key(p.badgeNumber, iso(day))] || { status: 'UNRECORDED' as Status }; const display = c.status === 'LEAVE' ? leaveCode(c.leaveType) : LABEL[c.status]; const hasDetails = c.status === 'LEAVE' || c.status === 'OB' || c.status === 'ABSENT'; return <td key={iso(day)} className={iso(day) === activeDay ? 'active-day-cell' : ''}><div className="status-cell"><button className={`status status-${c.status.toLowerCase()} ${c.status === 'LEAVE' && c.leaveType ? 'status-wide' : ''}`} onClick={() => cycleCell(p, day)}>{display}</button>{hasDetails && <button className="details-trigger" title="Optional details" aria-label={`Optional details for ${p.fullName}`} onClick={() => { setActiveDay(iso(day)); if (c.status === 'LEAVE') setLeavePicker({ person: p, day }); else openRangePicker(p, day, c.status); }}>⋯</button>}</div></td>; })}</tr>)}</tbody></table>}</div>
