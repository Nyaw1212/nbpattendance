...
export default function Home() {
  ...
  function choosePersonnelStatus(status: Status, label: string) { if (!statusPicker) return; const { person, day } = statusPicker; const k = key(person.badgeNumber, iso(day)); setCells(prev => ({ ...prev, [k]: { status } })); setActiveDay(iso(day)); setMessage(`${label} (${LABEL[status]}) selected for ${person.fullName}. Choose DATE for a range, or close this window to keep it for ${iso(day)} only.`); }
  ...
  {statusPicker && (() => { const current = cells[key(statusPicker.person.badgeNumber, iso(statusPicker.day))] || { status: 'UNRECORDED' as Status }; return <div className="picker-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) setStatusPicker(null); }}><section className="leave-picker" role="dialog" aria-modal="true" aria-label="Select personnel status"><div className="picker-head"><div><strong>Select personnel status</strong><span>{statusPicker.person.fullName} · {statusPicker.day.toLocaleDateString()} · Current: {LABEL[current.status] || current.status}</span></div><button onClick={() => setStatusPicker(null)} aria-label="Close">×</button></div><div className="leave-options"><button onClick={() => openRangeAfterModal(statusPicker, current.status, current.leaveType, () => setStatusPicker(null))}><b>DATE</b><span>Set date range for {LABEL[current.status] || current.status}</span></button>{PERSONNEL_STATUSES.map(option => <button key={option.code} onClick={() => choosePersonnelStatus(option.code, option.label)}><b>{option.code}</b><span>{option.label}</span></button>)}</div></section></div>; })()}
