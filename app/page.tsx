...
export default function Home() {
  ...
  function getCaseRange(employeeKey: string, cell: Cell, focusDay: string = activeDay) {
    const exact = pendingRanges.find(r => r.employeeKey === employeeKey && r.status === cell.status && (r.leaveType || '') === (cell.leaveType || '') && focusDay >= r.startDate && focusDay <= r.endDate);
    ...
    const activeIndex = weekDates.indexOf(focusDay);
    if (activeIndex < 0) return { startDate: focusDay, endDate: focusDay, days: 1 };
  ...
  function hoverFor(employeeKey: string, cell: Cell, focusDay: string) {
    const range = getCaseRange(employeeKey, cell, focusDay);
    setHoverRange({ employeeKey, status: cell.status, leaveType: cell.leaveType, startDate: range.startDate, endDate: range.endDate });
  }

  function sameHover(employeeKey: string, cell: Cell) {
    return !!hoverRange && hoverRange.employeeKey === employeeKey && hoverRange.status === cell.status && (hoverRange.leaveType || '') === (cell.leaveType || '');
  }

  function hoverFromPersonnel(employeeKey: string) {
    const activeCell = cells[key(employeeKey, activeDay)] || { status: 'UNRECORDED' as Status };
    if (activeCell.status !== 'PRESENT' && activeCell.status !== 'OFF' && activeCell.status !== 'ACT') {
      hoverFor(employeeKey, activeCell, activeDay);
      return;
    }
    setHoverRange({ employeeKey, status: activeCell.status, leaveType: activeCell.leaveType, startDate: activeDay, endDate: activeDay });
  }
